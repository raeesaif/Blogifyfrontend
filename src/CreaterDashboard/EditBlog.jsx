...
import { useGetSingleBlog, useUpdateBlog } from "@/hooks/useBlogapi";
...
export default function EditBlogPage() {
    ...
    const { data: blog, isLoading: isBlogLoading, error } = useGetSingleBlog(id);
    ...
    const { mutateAsync: updateBlog, isLoading: isSaving } = useUpdateBlog();
    ...
    useEffect(() => {
        if (blog) {
            reset({
                ...
                coverImageUrl: blog.coverImage || blog.coverImageUrl || "",
                category: blog.category || "",
            });
            if (blog.coverImage || blog.coverImageUrl) {
                setPreviewSrc(blog.coverImage || blog.coverImageUrl);
            }
        }
    }, [blog, reset]);
    ...
    const submitBlog = async (formValues, status) => {
        ...
        try {
            await updateBlog({ id, data: fd });
            navigate("/creater-dashboard");
        } catch (error) {
            toast.error(
                error?.response?.data?.message || "Could not update the blog. Please try again."
            );
        } finally {
                    ...
                    <Card className="bg-card border-border">
                        <CardContent className="p-12 text-center">
                            ...
                            <Button
                                variant="outline"
                                className="mt-4"
                                onClick={() => navigate("/creater-dashboard")}
                            >
                                Back to Dashboard
                            </Button>
                        </CardContent>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
                <div className="max-w-2xl">
                    <div className="mb-8">
                        ...
                        <p className="text-muted-foreground mt-1">Modify your story.</p>
                    </div>
